...
post.validate = function () {
    return function (req, res, next) {
        ...
        req.check('type', '400_3').isEnum([USER.signUpTypeEmail, USER.signUpTypePhone, USER.signUpTypePhoneId]);
        ...
        if (req.body.type == USER.signUpTypeEmail) {
            ...
            req.check('uid', '400_55').isId(USER.maxIdLength, USER.minIdLength);
            req.check('secret', '400_2').isAlphanumericPassword(USER.minSecretLength, USER.maxSecretLength);
        }
...
post.getUser = function () {
    return function (req, res, next) {
        ...
        // 이메일을 통한 로그인일 경우.
        if (req.body.type == USER.signUpTypeEmail || req.body.type == USER.signUpTypePhoneId) {
